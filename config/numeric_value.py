#  Unicorn - Embeddable Unicode Algorithms
#  Copyright (c) 2024-2025 Railgun Labs
#
#  This software is dual-licensed: you can redistribute it and/or modify
#  it under the terms of the GNU General Public License version 3 as
#  published by the Free Software Foundation. For the terms of this
#  license, see <https://www.gnu.org/licenses/>.
#
#  Alternatively, you can license this software under a proprietary
#  license, as set out in <https://railgunlabs.com/unicorn/license/>.

from typing import Dict
import zipfile
import json

from .config import Config
from .tools import Codespace
from .feature import Feature
from .basics import StorageSize, SerializationData

class NumericValueProperty(Feature):
    def pre_process(self, codespace: Codespace) -> None:
        codespace.register_property("numeric_value_offset", 0, StorageSize.UINT8)

    def process(self, archive: zipfile.ZipFile, codespace: Codespace, config: Config) -> SerializationData:
        file = archive.open('numeric_values.json')
        data = json.loads(file.read())
        mappings: Dict[str,int] = data["mappings"]
        source: str = data["source"]
        header: str = data["header"]
        size: int = data["size"]
        file.close()

        numval_property = codespace.get("numeric_value_offset")
        for cp, index in mappings.items():
            codespace.set(int(cp,16), numval_property, index)

        public_header = "#define UNICORN_FEATURE_NUMERIC_VALUE\n"
        return SerializationData(source, header, public_header, size)
